...
export default function AddPage() {
  ...
  const inputCls = "w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-200 outline-none focus:border-zinc-600";

  return (
    <div className="max-w-xl">
      <h1 className="text-3xl font-bold mb-2">Add Recipe</h1>
      <p className="text-zinc-400 mb-8">Manually add a recipe</p>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-xs text-zinc-500 mb-1 block">Cuisine</label>
            <input value={cuisine} onChange={(e) => setCuisine(e.target.value)} className={inputCls} placeholder="Italian" />
          </div>
          <div>
            <label className="text-xs text-zinc-500 mb-1 block">Prep Time</label>
            <input value={prepTime} onChange={(e) => setPrepTime(e.target.value)} className={inputCls} placeholder="15m" />
          </div>
          <div>
            <label className="text-xs text-zinc-500 mb-1 block">Cook Time</label>
        ...
        <div>
          <label className="text-xs text-zinc-500 mb-1 block">Ingredients (one per line)</label>
          <textarea value={ingredients} onChange={(e) => setIngredients(e.target.value)} className={`${inputCls} h-32`} placeholder={"2 cups flour\n1 tsp salt\n3 cloves garlic"} />
        </div>
        <div>
          <label className="text-xs text-zinc-500 mb-1 block">Instructions (one step per line)</label>
        ...
        <button
          ...
          className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-zinc-700 text-white px-6 py-3 rounded-xl text-sm font-medium transition-colors w-full"
        >
